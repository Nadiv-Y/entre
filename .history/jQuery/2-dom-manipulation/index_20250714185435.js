function generateBlueGoldStyle() {
  $(".blue-gold").css({ color: "blue", "background-color": "gold" });
}

generateBlueGoldStyle();

function generateMesurements() {
  $("#section-1-container").css({ hight: "300px", border : 'black solid 2px'});
}

generateMesurements ();

function hideButtons() {
  $(".section-1, .section-2, .setion-3").hide();
} 

hideButtons()

$('#display-1').click(function(){
    $('.section-1').show()
})
$('#display-2').click(function(){
    $('.section-2').show()
})
$('#display-3').click(function(){
    $('.section-3').show()
})

$('#toggle').click(function(){
    if($('#section-1-container').hadClass('blue-gold')){
         $(".blue-gold").css({ color: "red", "background-color": " " });
         $('#section-1-container').removeClass('blue-gold')
         $('#section-1-container').html('<p>remove</p>')
    }
$('#section-1-container').addClass('blue-gold')
generateBlueGoldStyle();
})