...
const selectedCoins = storedCoins.map((coin) => ({ id: coin.id, symbol: coin.symbol.toUpperCase() }));

$("#home").on("click", () => {
  $("#home-page").removeClass("d-none");
  $("#live-reports-page").addClass("d-none");
  clearInterval(liveReportInterval); //what does this do 
  getCoinData();
});
...
$("#live-reports").on("click", () => {
  ...
  renderLiveChart();//what does this do 
});
...
function getCoinData() {
  fetch("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd", {
    ...
    .then((coinsListRes) => {
      ...
      for (const coin of selectedCoins) { //need to understand this
        const toggle = document.getElementById(`toggle-${coin.id}`);
        if (toggle) toggle.checked = true;
      }

      document.addEventListener("click", function (event) { //figure out how to change
...
$(document).on("change", ".coin-toggle", function () {
  ...
  if (coinSelected) {
    ...
    // const i = selectedCoins.indexOf(coinId);
    const i = selectedCoins.findIndex((c) => c.id === coinId); //check if the value of the key id is the same as coinId
    if (i !== -1) {
...
function displaySelectionChoice(newCoin) {
  ...
  selectedCoins.forEach((coin) => {
    ...
    //       class="form-check-input modal-toggle"
    //       type="checkbox"
    //       id="modal-toggle-${coinId.id}"
    //       data-coin-id="${coinId.id}"
    //       data-new-coin="${sixthCoinId}"
    //       checked
    //     />
    //   </div>
    //  `;
...
$(document).on("change", ".modal-toggle", function () {
  ...
  if (!$(this).is(":checked")) {
    ...
    if (!selectedCoins.some((c) => c.id === newCoinId)) {
      selectedCoins.push({ id: newCoinId, symbol: newCoinSymbol.toUpperCase() }); //check if upperclass needed
    }
...
// $(document).on("change", ".modal-toggle", function () {
//   const coinToRemove = $(this).data("coin-id");
//   const newCoinToAdd = $(this).data("new-coin");

//   if (!$(this).is(":checked")) {
//     // const i = selectedCoins.indexOf(coinToRemove);
//     const i = selectedCoins.findIndex((c) => c.id === coinToRemove);
//     if (i !== -1) selectedCoins.splice(i, 1);

//     $(`#toggle-${coinToRemove}`).prop("checked", false);

//     if (!selectedCoins.includes(newCoinToAdd)) {
//       selectedCoins.push(newCoinToAdd);
//     }

//     $(`#toggle-${newCoinToAdd}`).prop("checked", true);
...
function renderLiveChart() {
  ...
  $("#chartContainer").html(""); //check if we write like this 

  chart = new CanvasJS.Chart("chartContainer", {
    animationEnabled: true,
    title: { text: "Live Coin Prices (USD)" }, // check how to display VS with coin names 
  ...
  if (liveReportInterval) clearInterval(liveReportInterval);// is it every time we click on live reports?

  liveReportInterval = setInterval(() => {
    const symbols = selectedCoins.map((coin) => coin.symbol).join(",");//what is this and if there is another way 
    fetch(
      `https://min-api.cryptocompare.com/data/pricemulti?fsyms=${symbols}&tsyms=USD` //only works for 5 coins 
    )
      .then((res) => res.json())
      .then((data) => {
        const now = new Date();
        chart.options.data.forEach((series, i) => {
          ...
          const price = data[coin]?.USD; //what is this 
          if (price) {
            series.dataPoints.push({ x: now, y: price });
            if (series.dataPoints.length > 30) { //what is this 
              series.dataPoints.shift(); //what is this 
            }
          }
        });
        chart.render();
...
function getColor(index) {
  ...
  return colors[index % colors.length]; // what does % do here 
}
